...
namespace VoxelPlay {
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour {
		...
		*/
        ...
        Mesh BakeMeshLighting(VoxelDefinition voxelDefinition, Color32 tintColor, VoxelPlaceholder placeholder) {

            BakedMesh bm = new BakedMesh {
                mesh = placeholder.originalMesh,
                tintColor = tintColor
            };
        ...
        Mesh BakeMeshLightingWithGather(VoxelDefinition voxelDefinition, Color32 tintColor, VoxelPlaceholder placeholder) {

            Vector3d worldPos = placeholder.transform.position;
            ...
            Color32[] originalColors32 = voxelDefinition.meshColors32;
            placeholder.originalMesh.GetVertices(modelMeshVertices);
            modelMeshColors.Clear();
            int vertexCount = placeholder.originalMesh.vertexCount;
                ...
                if (createGO) {
                    ...
                    if (createModel || placeholder.modelInstance == null) {
                        ...
                        if (voxelDefinition.gpuInstancing) {
                            ...
                            placeholder.originalMesh = placeholder.modelMeshFilter.sharedMesh;
                            if (voxelDefinition.meshColors32 == null && placeholder.originalMesh != null) {
                                voxelDefinition.meshColors32 = placeholder.originalMesh.colors32;
                            }
                        }
                    ...
                    if (voxelDefinition.gpuInstancing) {
                        ...
                        if (effectiveGlobalIllumination || chunk.voxels[voxelIndex].isColored) {
                            ...
                            if (mf != null) {
                                Mesh mesh = placeholder.originalMesh;
                                if (mesh != null) {
                                    Color32 tintColor = chunk.voxels[voxelIndex].color;
                                    if (voxelDefinition.computeLighting) {
                                        mf.sharedMesh = BakeMeshLightingWithGather(voxelDefinition, tintColor, placeholder);
                                    } else {
                                        tintColor.a = (byte)(chunk.voxels[voxelIndex].light + (chunk.voxels[voxelIndex].torchLight << 4));
                                        if (placeholder.lastMivTintColor.a != tintColor.a || placeholder.lastMivTintColor.r != tintColor.r || placeholder.lastMivTintColor.g != tintColor.g || placeholder.lastMivTintColor.b != tintColor.b) {
                                            mf.sharedMesh = BakeMeshLighting(voxelDefinition, tintColor, placeholder);
